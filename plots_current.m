...
for s = 1:length(recordings_file)-2 
    if strcmp(recordings_file(s).name, '.') || strcmp(recordings_file(s).name, '..')
        continue
    elseif length(recordings_file(s).name) > 3 && recordings_file(s).name(1,5) == '_'
        continue
    end
